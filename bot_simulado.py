...
# --- FORZADO TOTAL: FILTRO EN NEGATIVO PARA TRADES INSTANTÁNEOS ---
TAKER_FEE = 0.0010       
MIN_PROFIT = -1.0        # Permite ejecutar trades de inmediato aunque el mercado dé pérdidas
MAX_PROFIT = 5.0      
...
data_compartida = {
    ...
    "total_datos_mb": 0.0,
    "record_max_profit": -99.0,
    "record_min_profit": 99.0,
    "top_rutas_texto": "Cargando rutas...",
    "transacciones_texto": "Esperando spread...",
    ...
    while True:
        ...
        try:
            ...
            for i, (r, p, prc) in enumerate(top_3):
                color = "#02c076" if p >= 0.35 else "#f84960"
            ...
            if mejor_profit >= MIN_PROFIT:
                ...
                tx_linea = f"<div style='display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #2b3139;font-size:13px;'><span style='color:#848e9c;'>{time.strftime('%H:%M:%S')}</span><span style='font-weight:bold;'>{mejor_ruta_texto}</span><span style='color:#f84960;font-weight:bold;'>{mejor_profit:.3f}%</span><span style='font-weight:bold;'>${CAPITAL_SIMULADO:.2f}</span></div>"
...
@app.route('/datos_raw')
def datos_raw():
    color_profit = "#02c076" if data_compartida['mejor_profit'] >= 0.35 else "#f84960"
    return json.dumps({
